    ...
    <div class="container extra-class-container" style="margin: 1.5rem 0px !important; padding: 0 1rem;">
        <div class="row">
            <div class="col-md-4 col-6 header-col">
                <h3 class="heading">Affiliate Member</h3>
            </div>
            <div class="col-md-8 col-6">
                <div class="btn-text m-0 add-new-cat " style="display: flex; justify-content: flex-end;">
                    <!-- <router-link to="/affiliate-create" class="import2 add-cc hover-cc">Create Affiliate</router-link> -->
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6 col-12 right-side-search mb-2">
                    <div class="search-input position-relative">
                        <input type="text" class="form-control" placeholder="Search by Name, Email and Referral Number"
            ...
            <div v-else>
                <div class="table-responsive">
                    <table class="table table-striped table-responsive">
                        ...
                        <tbody>
                            <tr v-for="(item, index) in user">
                                ...
                                <td class="text2"><span class="payment_status">{{ item.position }}</span></td>
                                <td class="text2">{{ item.phone }}</td>
                                <td class="text2">{{ item.referral_id }}</td>
                                <td class="text2 d-flex gap-1">
                                    <router-link :to="{ name: 'affiliateEdit', params: { id: item.id } }">
                    ...
                    <Bootstrap5Pagination v-if="!searchQuery" :show-disabled=true :limit="5" :data="currentPagination"
                        @pagination-change-page="fetchUsersShowAgent" />
                </div>
...
export default {
    ...
    methods: {
        ...mapActions('auth', ['fetchUsersShowAgent', 'deleteUsers', 'fetchUserSearch',]),
        UsersSearchWord() {
    ...
    computed: {
        ...mapState('auth', ['isAuthenticated', 'user', 'pagination']),
        currentPagination() {
