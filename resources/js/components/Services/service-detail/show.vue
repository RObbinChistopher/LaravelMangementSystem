<template>
    <div class="container extra-class-container" style="margin: 1.5rem 0px !important">
        ...
        <div class="parent-accordion p-3" id="parent-accordion">
            ...
            <!-- Loading Spinner -->
            <div class="spinner2 d-flex justify-content-center" v-if="serviceDetailsLoading">
            ...
            <div v-else>
                <table class="table table-striped table-responsive">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in serviceDetails">
                            <th scope="row">{{ index + 1 }}</th>
                            <td class="text2">{{ item.name }}</td>
                            <td class="text2">{{ item.price }}$</td>
                            <td class="text2">{{ item.tax }}%</td>
                ...
                <div v-if="serviceDetails && serviceDetails.length === 0" class="text"
...
export default {
    ...
    methods: {
        ...mapActions('service', ['fetchAllServicesDetails', 'fetchdeleteServicesDetails']),
        confirmDelete(id) {
            ...
            this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    this.fetchdeleteServicesDetails(id).then(() => {
                        this.$swal.fire({
                            title: "Deleted!",
                    ...
                    this.fetchAllServicesDetails(1);
                }
            });
        }
    },
    ...
    computed: {
        ...mapState('service', ['serviceDetails', 'serviceDetailsLoading', 'pagination']),
        currentPagination() {
